layui.use(['form', 'layer', 'jquery_cookie'], function () {
    ...
    $.post(ctx + "/user/queryAllCustomerManager", function (res) {
        for (var i = 0; i < res.length; i++) {
            if ($("input[name='man']").val() == res[i].id) {
                $("#assigner").append("<option value=\"" + res[i].id + "\" selected='selected' >" + res[i].userName + "</option>");
            } else {
                $("#assigner").append("<option value=\"" + res[i].id + "\">" + res[i].userName + "</option>");
            }
        }
        //重新渲染
    ...
    form.on("submit(addOrUpdateCustomerServe)", function (data) {
        ...
        //弹出loading
        $.post(ctx + "/customer_server/saveOrUpdateCustomerServer", data.field, function (res) {
            if (res.code == 200) {
                setTimeout(function () {

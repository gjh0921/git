    ...
    table.on('toolbar(customers)', function (obj) {
        ...
        switch (obj.event) {
            ...
            case "order":
                ...
                break;
            case "link":
                openCusLinkInfoDialog(checkStatus.data);
                break;
            case "recode":
                openCuscontcatDialog(checkStatus.data);
                break;
        }
    });
...
// 打开订单查看页面
    ...
    // 打开联系人管理页面
    function openCusLinkInfoDialog(datas) {
        if (datas.length == 0) {
        ...
        var url = ctx + "/customer/linkmanPage?cid=" + datas[0].id;
        layui.layer.open({
            title: "联系人管理",
            type: 2,
            area: ["700px", "500px"],
    ...
    // 打开交往记录页面
    function openCuscontcatDialog(datas) {
        if (datas.length == 0) {
            layer.msg("请选择待查看联系人对应客户!", {icon: 5});
            return;
        }
        ...
        var url = ctx + "/customer/cusContcatPage?cid=" + datas[0].id;
        layui.layer.open({
            title: "联系人管理",
            type: 2,
            area: ["700px", "500px"],

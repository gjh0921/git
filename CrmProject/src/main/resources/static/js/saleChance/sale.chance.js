layui.use(['table', 'layer'], function () {
    ...
    var tableIns = table.render({
        ...
        cols: [[
            {type: "checkbox", fixed: "center", align: "center"},
           ...
           /* {
            ...
            /*{
                templet: "<div>{{layui.util.toDateString(d.assignTime,'yyyy-MM-dd HH:mm:ss')}}</div>",
                title: '分配时间',
                align: 'center'
            },*/
    ...
    function formatterState(state) {
        if (state == 0) {
            return "<div style='color:blue '>未分配</div>";
        } else if (state == 1) {
    ...
    function formatterDevResult(value) {
         ...
         */
        if (value == 0) {
            return "<div style='color: blue'>未开发</div>";
        } else if (value == 1) {
    ...
    table.on("tool(saleChances)", function (obj) {
        ...
        if (layEvent === "edit") {
            ...
            layer.confirm('确定删除当前数据？', {icon: 3, title: "机会数据管理"}, function () {
